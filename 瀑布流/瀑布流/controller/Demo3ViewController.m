...
@interface Demo3ViewController () <UICollectionViewDelegate, UICollectionViewDataSource, CandyFlowLayoutDelegate>
...
- (void)viewDidLoad {
    ...
    // 在每个数组前面塞一个"全部"
    [self.contentArray enumerateObjectsUsingBlock:^(NSMutableArray *obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        [obj addObject:@"全部"];
        [obj insertObject:@"全部" atIndex:0];
    }];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.contentLab.text = self.contentArray[indexPath.section][indexPath.item];
...
- (CGSize)sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    CGSize size = [self.contentArray[indexPath.section][indexPath.item] boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:17]} context:nil].size;
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        layout.style = CandyFlowLayoutStyleSpecial;
